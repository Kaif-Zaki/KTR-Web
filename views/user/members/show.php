<link rel="stylesheet" href="<?= url('/public/css/user/members.css') ?>">

<div class="member-show-wrapper">
    
    <a href="<?= url('/members') ?>" class="back-link reveal-text">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
        <span>Back to Members</span>
    </a>

    <div class="member-profile-card reveal-container">
        
        <div class="profile-visual">
            <div class="profile-avatar-large <?= !$member['image_path'] ? 'no-img' : '' ?>">
                <?php if ($member['image_path']): ?>
                    <img src="<?= url('/public/images/members/' . $member['image_path']) ?>" alt="<?= e($member['name']) ?>">
                <?php else: ?>
                    <span><?= strtoupper(substr($member['name'], 0, 1)) ?></span>
                <?php endif; ?>
            </div>
        </div>

        <div class="profile-info">
            <h1 class="reveal-text"><?= e($member['name']) ?></h1>
            <p class="profile-occupation reveal-text"><?= e($member['occupation']) ?></p>

            <div class="profile-details-grid">
                <div class="detail-item reveal-text">
                    <label>Personal Member</label>
                    <p>Welfare Society</p>
                </div>
                <div class="detail-item reveal-text">
                    <label>Email Address</label>
                    <p><?= e($member['email']) ?></p>
                </div>
            </div>

            <div class="profile-actions">
                
                <?php if ($member['whatsapp']): ?>
                <div class="contact-section reveal-container">
                    <a href="https://wa.me/<?= e(preg_replace('/[^0-9]/', '', $member['whatsapp'])) ?>" 
                       target="_blank" 
                       rel="noopener"
                       class="wa-btn"
                       style="padding: 14px 32px; font-size: 1rem;">
                        <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.09.543 4.054 1.492 5.762L0 24l6.384-1.472A11.954 11.954 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.818 9.818 0 01-5.007-1.37l-.36-.213-3.724.858.887-3.63-.234-.373A9.818 9.818 0 1112 21.818z"/></svg>
                        <span>Message on WhatsApp</span>
                    </a>
                </div>
                <?php endif; ?>

                <?php if ($member['social_links']): ?>
                <div class="social-section reveal-container">
                    <h4>Connect on Social Media</h4>
                    <div class="social-icons-list">
                        <?php 
                            $links = json_decode($member['social_links'], true); 
                            if (is_array($links)):
                                foreach ($links as $link): 
                                    if (empty($link['url'])) continue;
                                    $url = $link['url'];
                                    if (!preg_match("~^(?:f|ht)tps?://~i", $url)) {
                                        $url = "https://" . $url;
                                    }
                                    $platform = strtolower($link['platform']);
                        ?>
                            <a href="<?= e($url) ?>" target="_blank" rel="noopener" class="social-icon-link social-icon-link--<?= e($platform) ?>" title="<?= ucfirst(e($platform)) ?>">
                                <?php if($platform === 'facebook'): ?>
                                    <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                <?php elseif($platform === 'instagram'): ?>
                                    <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.044.058-1.265.07-1.645.07-4.85 4.919-4.919 1.265-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.847.068-4.307.199-6.913 2.766-7.155 7.155-.054 1.18-.068 1.588-.068 4.847 0 3.259.014 3.668.068 4.848.242 4.389 2.848 6.956 7.155 7.155 1.18.054 1.588.068 4.847.068 3.26 0 3.668-.014 4.848-.068 4.307-.199 6.913-2.766 7.155-7.155.054-1.18.068-1.589.068-4.848 0-3.259-.014-3.667-.068-4.847-.241-4.307-2.847-6.914-7.155-7.155-1.18-.054-1.588-.068-4.848-.068zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.162 6.162 6.162 6.162-2.759 6.162-6.162-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                                <?php elseif($platform === 'linkedin'): ?>
                                    <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z"/></svg>
                                <?php elseif($platform === 'twitter' || $platform === 'x'): ?>
                                    <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                                <?php else: ?>
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                                <?php endif; ?>
                            </a>
                        <?php 
                                endforeach; 
                            endif;
                        ?>
                    </div>
                </div>
                <?php endif; ?>
            </div>
        </div>
    </div>
</div>

